      ...
      try {
        ...
        if (quizRes.success && quizRes.data) {
          ...
          if (Array.isArray(fetchedQuiz.class)) {
            classValue = fetchedQuiz.class.map((c: any) =>
              typeof c === "object" && c !== null ? c._id : c,
            );
          } else {
            classValue =
              typeof fetchedQuiz.class === "object" &&
              fetchedQuiz.class !== null
  ...
  const handleSetCorrectOption = (qIndex: number, optIndex: number) => {
    setQuizData((prev) => {
      ...
      if (qType === "mcq" || qType === "true-false") {
        ...
        updatedQs[qIndex].options[optIndex].isCorrect =
          !updatedQs[qIndex].options[optIndex].isCorrect;
      }
      return { ...prev, questions: updatedQs };
    });
        ...
        <div className="flex items-center gap-3 mb-8 py-4">
          ...
          <div>
            ...
            <p className="font-semibold text-gray-700 text-sm">
              Update your SL Accounting assessment
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="text-sm text-gray-700">
                              {cls.name}{" "}
                              {cls.batch
                                ? `(${typeof cls.batch === "string" ? cls.batch : (cls.batch as any).name})`
                                : ""}{" "}
                              - {cls.type}
                            </span>
                          </label>
          ...
          <div className="lg:col-span-2 space-y-6">
                      ...
                      {q.options.map((opt, optIndex) => (
                        <div key={optIndex} className="flex items-center gap-3">
                          <input
                            ...
                            onChange={() =>
                              handleSetCorrectOption(qIndex, optIndex)
                            }
            ...
            <button
              type="button"
              onClick={() => navigate("/admin/quizzes")}
              className="px-10 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 transition flex items-center gap-2"
            >
              Cancel
            </button>
            <button
