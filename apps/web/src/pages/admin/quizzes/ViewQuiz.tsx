...
const ViewQuiz: React.FC = () => {
  ...
  if (Array.isArray(quiz.class)) {
    classNameDisplay = quiz.class
      .map((c: any) =>
        typeof c === "object" && c !== null
          ? c.name || c.className || c._id
          : c,
      )
      .join(", ");
  } else {
  ...
  return (
    <div className="p-6 min-h-screen">
      <div className="max-w-5xl mx-auto">
        {/* Header Actions */}
        <div className="flex items-center justify-between mb-8 sticky top-0  z-10 py-4 ">
          <div>
            ...
            <p className="font-semibold text-gray-700 text-sm ml-12">
              Review the assessment details and correct answers
            </p>
          </div>
          <div className="flex gap-3">
            <Link
              to={`/admin/quizzes/edit/${quiz._id}`}
              className="px-8 py-2 bg-[#0A5B70] hover:bg-[#08475A] font-bold text-white rounded-lg  transition flex items-center gap-2"
            >
              Edit Quiz
            </Link>
          </div>
...
<div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200">
  <h2
    className="text-lg font-semibold flex items-center gap-2 mb-4 pb-3 border-b border-gray-200"
    style={{ color: "#0A5B70" }}
  >
    <Settings
      size={18}
  ...
  <ul className="space-y-2 text-sm">
    ...
    <li className="flex items-center gap-2 text-gray-700">
      {quiz.settings?.showResultImmediately ? (
        <CheckCircle2 size={16} className="text-green-500" />
      ) : (
        <XCircle size={16} className="text-red-400" />
      )}
              ...
              {q.explanation && (
                <div className="ml-11 mt-4 p-4 bg-blue-50/50 border border-blue-100 rounded-lg flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-800 whitespace-normal">
                      {q.explanation}
                    </p>
                  </div>
                </div>
