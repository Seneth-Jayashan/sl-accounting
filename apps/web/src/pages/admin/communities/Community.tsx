...
import { 
  ...
  ShieldCheck, X, FileText, Loader2, Trash2, ArrowLeft
} from "lucide-react";
...
export default function AdminCommunity() {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      else setIsSidebarOpen(!selectedClass); // Show sidebar on mobile ONLY if no class selected
    };
  ...
  useEffect(() => {
    ...
    // Mobile: Close sidebar when class selected
  ...
  }, [selectedClass?._id, user?._id]); // Removed isMobile dependency to prevent re-join loops
  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] md:h-[calc(100vh-2rem)] bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200 relative">
      ...
      <AnimatePresence initial={false}>
        {isSidebarOpen && (
            <motion.div 
                ...
                className={`
                    absolute inset-0 z-20 bg-white md:relative md:inset-auto md:w-80 lg:w-96 
      ...
      <div className="flex-1 flex flex-col bg-white relative w-full">
        {selectedClass ? (
          <>
            <div className="h-16 border-b border-gray-100 flex items-center justify-between px-4 sm:px-6 bg-white shrink-0 z-10">
              <div className="flex items-center gap-3">
                  {/* Mobile Back Button */}
                  {isMobile && (
                      <button onClick={() => setIsSidebarOpen(true)} className="p-2 -ml-2 hover:bg-gray-100 rounded-full text-gray-500">
                          <ArrowLeft size={20} />
                      </button>
                  )}
                  
                  <div>
                    <h2 className="font-bold text-gray-800 truncate max-w-[200px] sm:max-w-md">{selectedClass.name}</h2>
                ...
                {loadingChat ? (
                    ...
                    messages.map((msg, idx) => {
                        ...
                        return (
                            ...
                            >
                                <div className={`flex max-w-[85%] sm:max-w-[70%] gap-2 sm:gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                                    {/* Avatar */}
                                    ...
                                    <div className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                                        <div className="flex items-center gap-2 mb-1">
                                            <span className={`text-[10px] sm:text-xs font-bold ${isAdmin ? 'text-brand-cerulean' : 'text-gray-600'}`}>
                                                {msg.senderName} {isAdmin && !isMe && "(Admin)"}
                                            </span>
                                            
                                            {/* DELETE BUTTON (Mobile: Always visible if yours, Desktop: Hover) */}
                                            <button 
                                                onClick={() => handleDeleteMessage(msg._id)}
                                                className="opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity p-1 text-gray-400 hover:text-red-500 bg-gray-100 rounded-full ml-1"
                                            >
                                                <Trash2 size={10} />
                                            </button>
                                        </div>
                                        
                                        <div className={`p-1.5 shadow-sm rounded-2xl ${isMe ? 'bg-brand-cerulean text-white rounded-tr-none' : 'bg-white border border-gray-200 text-gray-700 rounded-tl-none'}`}>
                                            {msg.attachments?.map((att, i) => (
                                                <div key={i} className="mb-1">
                                                    {att.fileType === 'image' ? (
                                                        <a href={BASE_API_URL + att.url} target="_blank" rel="noopener noreferrer">
                                                            <img src={BASE_API_URL + att.url} alt="att" loading="lazy" className="max-w-[200px] sm:max-w-[250px] max-h-[200px] rounded-lg bg-black/5 object-cover" />
                                                        </a>
                                                    ) : (
                                            ...
                                            {msg.message && <div className="px-3 py-2 text-sm whitespace-pre-wrap break-words">{msg.message}</div>}
                                        </div>
            ...
            <div className="p-3 sm:p-4 bg-white border-t border-gray-100 relative">
                ...
                <form onSubmit={handleSendMessage} className="flex items-center gap-2 sm:gap-3">
                    ...
                    <label htmlFor="admin-chat-upload" className="cursor-pointer p-2 bg-gray-50 rounded-full text-gray-500 hover:text-brand-cerulean hover:bg-gray-100 transition-colors">
                        <Paperclip size={20}/>
                    </label>
                    <input 
                        type="text" 
                        ...
                        className="flex-1 bg-gray-100 border-none rounded-full px-4 py-2.5 outline-none text-sm focus:ring-2 focus:ring-brand-cerulean/20 transition-all"
                    />
                    <button type="submit" disabled={!newMessage && !selectedFile} className="p-2.5 bg-brand-prussian text-white rounded-full hover:bg-brand-cerulean transition-colors shadow-lg disabled:opacity-50 disabled:shadow-none">
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-gray-300 p-6 text-center">
             ...
             <p className="text-lg font-bold text-gray-400">Select a Class</p>
             <p className="text-sm">Choose a class from the list to start chatting.</p>
             {isMobile && (
                 <button onClick={() => setIsSidebarOpen(true)} className="mt-6 px-6 py-2 bg-brand-prussian text-white rounded-full text-sm font-bold shadow-lg">
                     Open Class List
                 </button>
             )}
          </div>
