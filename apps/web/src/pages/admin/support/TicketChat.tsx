...
export default function TicketChatAdmin() {
		...
		TicketService.getAllTickets()
			.then((items) => {
				...
				if (paramId && sorted.some((t) => t._id === paramId)) {
					...
					if (!paramId) navigate(`/admin/chat/ticket/${sorted[0]._id}`, { replace: true });
				}
			})
			.catch((e) => {
				console.error(e);
	...
	const stats = useMemo(() => {
		...
		const open = tickets.filter((t) => !String(t.status ?? "").toLowerCase().includes("close"));
	...
	const handleStatusChange = async (nextStatus: string) => {
		...
		if (isResolved && nextStatus !== "Closed" && nextStatus !== currentStatus) {
			window.alert("This ticket was marked Resolved by the user. You can only Close it.");
			return;
		}

		// Confirm before closing
		if (nextStatus === "Closed") {
			const ok = window.confirm("Close this ticket? This will prevent further updates unless reopened by an admin.");
			if (!ok) return;
		}
	...
	const handleDelete = async () => {
		...
		const confirmed = window.confirm("Delete this ticket and its chat messages?");
		if (!confirmed) return;
				...
				<main className="p-4 lg:p-8 pb-24 lg:pb-10 overflow-x-hidden flex justify-center">
					<div className="w-full max-w-7xl space-y-6">
						<div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
							<div>
								<h1 className="text-3xl font-semibold text-[#0b2540]">Ticket Chats</h1>
								<p className="text-sm text-gray-600">Review every ticket thread and reply as admin.</p>
							</div>
							<div className="flex items-center gap-2 text-sm text-gray-700">
								<span className="px-3 py-1 rounded-full bg-white border shadow-sm">Total: {stats.total}</span>
								<span className="px-3 py-1 rounded-full bg-white border shadow-sm">Open-ish: {stats.open}</span>
							</div>
						</div>
						...
						{loading ? (
							<div className="flex items-center justify-center py-16 text-gray-600">Loading tickets...</div>
						) : (
												...
												return (
													<li
														...
														className={`p-4 cursor-pointer transition hover:bg-gray-50 ${
															isActive ? "bg-[#0b2540]/5 border-l-4 border-[#0b2540]" : ""
														}`}
														onClick={() => {
														if (isActive) {
													...
													>
														...
														<div className="mt-2 text-xs text-gray-500">Priority: {t.priority || "Low"}</div>
													</li>
												);
											})}
										</ul>
									...
									<div className="rounded-2xl border bg-white shadow-sm p-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
										<div>
											...
											{loadingTicket ? (
												...
												<div className="text-sm text-gray-600">Select a ticket to inspect.</div>
											)}
										</div>
										{selectedTicket && (
											<div className="flex flex-col md:flex-row gap-2 md:items-center">
												<label className="text-sm text-gray-600 flex items-center gap-2">
													...
													<select
														...
														disabled={statusUpdating || deleting || String(selectedTicket.status || "").toLowerCase() === "closed"}
													>
														{STATUS_OPTIONS.filter((opt) => {
															const current = selectedTicket.status || "Open";
															if (opt === "Resolved" && current !== "Resolved") return false; // admin can't set to Resolved
															if (opt === "Closed" && current !== "Resolved" && current !== "Closed") return false; // show Closed only after Resolved
															return true;
														}).map((opt) => (
												...
												{(selectedTicket.status || "").toLowerCase() === "closed" ? (
													...
													<span className="text-xs text-gray-500">Delete available only when ticket is Closed</span>
												)}
											</div>
										)}
									</div>
