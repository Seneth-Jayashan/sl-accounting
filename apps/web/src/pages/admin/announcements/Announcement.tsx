...
export default function AnnouncementsAdmin() {
  ...
  const handleEditClick = (ann: AnnouncementData) => {
    ...
    setFormData({
      ...
      content: ann.content,
      classId:
        ann.class && typeof ann.class === "object"
          ? (ann.class as any)._id
          : (ann.class as string),
    });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.classId) {
      showError(new Error("Please select a class"), "Please select a class");
      return;
    }
      ...
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-xs sm:text-sm font-medium">
            Broadcast updates to specific academic modules.
          </p>
        </div>
        <button
        ...
        <div className="grid grid-cols-1 gap-4 sm:gap-5">
          {announcements.length > 0 ? (
            announcements.map((ann) => (
              <div
                key={ann._id}
                className="bg-white p-5 sm:p-7 rounded-2xl sm:rounded-[2.5rem] border border-brand-aliceBlue hover:border-brand-cerulean/10 transition-all group shadow-sm flex flex-col gap-4"
              >
                {/* Header Row */}
                ...
                {/* Content */}
                <div>
                  <h3 className="text-base sm:text-lg font-semibold text-brand-prussian mb-2 group-hover:text-brand-cerulean transition-colors">
                    {ann.title}
                  </h3>
                  <p className="text-gray-500 text-xs sm:text-sm leading-relaxed font-normal whitespace-pre-line">
                    {ann.content}
                  </p>
                </div>
              </div>
            ...
            <div className="bg-white rounded-3xl p-10 sm:p-20 text-center border-2 border-dashed border-brand-aliceBlue">
              <Megaphone
                className="mx-auto text-brand-aliceBlue/40 mb-4"
                size={40}
                strokeWidth={1.5}
              />
              <p className="text-gray-400 font-medium text-sm">
                No announcements found for this selection.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-brand-prussian/40 backdrop-blur-sm">
          <div className="bg-white w-full max-w-lg rounded-2xl sm:rounded-[3rem] p-6 sm:p-10 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
              <div>
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-2 sm:mb-3">
