...
const getImageUrl = (path?: string): string => {
  ...
  // Securely construct URL preventing double slashes
...
export default function PublicClassesPage() {
    ...
    const initData = async () => {
      ...
      try {
        ...
        const fetchedClasses: ClassData[] = Array.isArray(classRes)
          ? classRes
          : (classRes as any).classes || [];
        setClasses(fetchedClasses);

        // Process Batches with Deduplication
        if (batchRes.batches && batchRes.batches.length > 0) {
          setBatches(batchRes.batches);
        } else {
          // Fallback: Extract unique batches from class data safely
  ...
  }, [classes, search, selectedBatch, selectedLevel]);
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {[1,2,3].map(i => (
                    <div key={i} className="bg-white rounded-[2rem] h-[450px] animate-pulse border border-gray-100 shadow-sm overflow-hidden flex flex-col">
            ...
            <motion.div 
                ...
                className="text-center py-20 bg-white rounded-[2.5rem] shadow-sm border border-gray-100 max-w-2xl mx-auto"
            >
                ...
                <AnimatePresence mode="popLayout">
                    {filteredClasses.map((cls) => {
                        ...
                        return (
                            ...
                            >
                                {/* Wrapper for entire card clickability */}
                                <div onClick={() => navigate(`/classes/${cls._id}`)} className="cursor-pointer flex flex-col h-full">
                                    ...
                                    <div className="relative h-56 overflow-hidden bg-brand-aliceBlue">
                                        ...
                                        <div className="absolute top-4 left-4 flex flex-wrap gap-2">
                                            <span className="bg-white/95 backdrop-blur-md text-brand-prussian text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-widest shadow-sm">
                                                {cls.level}
                                            </span>
                                    ...
                                    <div className="p-6 flex-1 flex flex-col">
                                        ...
                                        <p className="text-gray-500 text-sm line-clamp-2 mb-6 font-sans leading-relaxed flex-1">
                                            {cls.description}
                                        </p>
