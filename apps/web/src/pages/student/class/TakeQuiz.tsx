...
const TakeQuiz: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchQuiz = async () => {
      if (!id) {
        showError(new Error("Quiz ID is missing from the URL!"));
        navigate(-1);
        return;
      }
  ...
  const handleFinalSubmit = useCallback(
    async (isTimeOut = false) => {
      ...
      try {
        ...
        if (response.success) {
          ...
          showSuccess(message);
          navigate(`/student/quizzes/result/${submissionId}`, {
            replace: true,
          });
        }
      } catch (error: any) {
    ...
    [submissionId, navigate, quiz, showError, showSuccess]
  );
          ...
          <div className="p-8">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
              ...
              <div className="bg-purple-50 p-4 rounded-2xl text-center border border-purple-100">
                <p className="text-xs text-purple-500 uppercase font-bold mb-1">
                  Attempts
                </p>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex gap-4 mb-8">
              ...
              <div>
                ...
                <ul className="text-sm text-amber-700 space-y-1 list-disc ml-4">
                  ...
                  <li>
                    The exam will automatically submit when the timer reaches
                    zero.
                  </li>
                  <li>
                    Keep your calculator and rough paper ready for accounting
                    calculations.
                  </li>
                </ul>
              </div>
  ...
  return (
    ...
    >
      <header className="bg-white border-b border-gray-200 sticky top-0 z-40 px-6 py-4 flex items-center justify-between shadow-sm">
        <h1 className="text-xl font-bold text-brand-prussian hidden md:block">
          {quiz.title}
        </h1>
        ...
        >
          <Clock
            size={20}
            className={isTimeRunningOut ? "text-red-500" : "text-gray-500"}
          />
          {formatTime(timeLeft)}
        </div>
          ...
          <div className="space-y-3 mt-auto mb-8">
            {currentQ.questionType === "short-answer" ? (
              <textarea
                ...
                onChange={(e) =>
                  handleShortAnswer(currentQ._id!, e.target.value)
                }
                  ...
                  >
                    <div
                      className={`w-6 h-6 flex items-center justify-center rounded-full border-2 flex-shrink-0 ${
                        isSelected
                          ? "border-brand-cerulean bg-brand-cerulean text-white"
                          : "border-gray-300"
                      }`}
                    >
                    ...
                    <input
                      type={
                        currentQ.questionType === "multi-select"
                          ? "checkbox"
                          : "radio"
                      }
                      name={`q-${currentQ._id}`}
            ...
            {quiz.questions.map((q, idx) => {
              ...
              return (
                <button
                  ...
                  disabled={
                    !quiz.settings.allowBacktrack && idx < currentQIndex
                  }
